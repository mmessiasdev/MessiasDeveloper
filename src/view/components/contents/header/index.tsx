...
const Header = () => {
  const [user, setUser] = useState<GithubUserModel>();

  useEffect(() => {
    fetch("https://api.github.com/users/mmessiasdev").then(response => response.json()).then(data => setUser(data));
    console.log(user);
  }, [])
            ...
            {user &&
              <>
                <Container pad="35px 0px">
                  ...
                  <BaseText tag="h5">@{user.login}</BaseText>
                  <BaseText tag="h3">{user.name}</BaseText>
                  <BaseText tag="h4">{user.public_repos} repositórios</BaseText>
                  <BaseText tag="h4">{user.location}</BaseText>
                </Container>
              </>
            }
